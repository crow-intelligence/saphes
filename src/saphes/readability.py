...
def word_length(word: str, *, policy: LengthPolicy | LengthFn = "nfc") -> int:
    ...
    Contract:
        ...
        - ``"nfc"`` is idempotent on already-composed text, so it is free for
          callers who already normalise.
...
def interpret_lix(score: float) -> LixBand:
    ...
    Contract:
        - Calibrated for Swedish/Germanic prose at ``long_word_threshold=6``
          ...
          :attr:`LixResult.band` returns ``None`` there rather than mislabel it.
...
def lix(
    ...
    **Pass surface forms, not lemmas.** Word length is the signal; lemmatising
    ...
    Contract:
        - Empty and whitespace-only tokens are dropped before counting and the
          number is recorded as ``dropped_empty``.
        - ``long_word_threshold=0`` makes every word long, so the second term is
          exactly ``100.0``.
