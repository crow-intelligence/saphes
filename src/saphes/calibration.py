...
def length_curve(
    ...
    *,
    ...
    Contract:
        ...
        - Weighting is by token, never by type. **Passing all-1 counts yields
          the type-weighted curve, which is not what you want** — rare types are
          long, so it runs far above the truth while still looking plausible.
...
def match_threshold(
    ...
    *,
    ...
    Contract:
        - The chosen threshold minimises ``abs(target_share - reference_share)``
          over the target curve; ties go to the lower threshold.
        - ``bracket`` spans the two consecutive thresholds the reference share
          falls between, so a result sitting on a boundary is visible rather
          than rounded away.
